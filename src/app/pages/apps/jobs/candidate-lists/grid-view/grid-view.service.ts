...
function sort(countries: GridViewModel[]): GridViewModel[] {
  // if (direction === '') {
  return countries;
  // } else {
...
function matches(country: GridViewModel, term: string, pipe: PipeTransform) {
  return country.id.toLowerCase().includes(term.toLowerCase())
    ...
    || country.ratingCount.toLowerCase().includes(term.toLowerCase());
}
...
export class GridViewService {
  ...
  private _countries$ = new BehaviorSubject<GridViewModel[]>([]);
